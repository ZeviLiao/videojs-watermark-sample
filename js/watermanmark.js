(function () {
  ...
  videojs.registerPlugin("ITMAN_watermark", function (settings) {
    ...
    let options, player, video, img, link, marquee, p;
    ...
    if (!div) {
      ...
      options.className === "vertical-center"
        ? (div.className = options.className)
        : (div.className = "vertical-center " + options.className);
    } else {
    ...
    if (options.textContent) {
      ...
      p = marquee.childNodes[1];
      p.innerHTML = options.textContent
    ...
    if (options.position === "top-left" || options.position === "left-top") {
      ...
      // div.style.top = video.offsetHeight / 2 + "px";
      // div.style.left = video.offsetWidth / 2 + "px";
    }
